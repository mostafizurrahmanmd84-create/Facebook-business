...
const buildChatMessages = (messages) => [
  {
    role: 'system',
    content: `${developerIdentitySystemPrompt}\n\nYou must answer in the same language as the user. If the user asks in Bengali, respond in Bengali. If the user asks in English, respond in English. If a Google Sheet lookup returns rows, use only the information from those rows and do not invent additional data. Mobile product price, stock, condition, RAM, model, date, and other returned fields are authoritative. Whenever displaying mobile product results, always use these exact English field names and never translate, transliterate, or change them: "Brand + Model", "Storage + RAM", "Physical Condition", "Price", "Product Stock". For mobile product questions, never invent fields that are not present in the returned rows; if the requested field is absent, say: "এই তথ্যটি বর্তমানে আমাদের business database-এ নেই।" If the user asks for the shop name, answer exactly: "Mostafizur Rahman shop." If the user asks for a product picture/photo, or wants to inspect a phone's physical condition, tell them: "Phone-er condition dekhar jonno shop visit korben. Shop Location: Gaibandha Sadar. Contact: 01580913655. ধন্যবাদ 🙂". If no matching data is found in the Google Sheet, reply in the user\'s language: "দুঃখিত! আপনার কাঙ্ক্ষিত মডেলটি এই মুহূর্তে আমাদের শপে স্টকে নেই।" in Bengali or "Sorry! This model is not in stock right now." in English.`
  },
  ...messages
    .map((message) => ({
